...
| | |
| --- | --- |
| **浏览与搜索** | 全部内置 + 自定义插件;搜索支持名称、中文关键词、分类、别名(输 `gst` 直达 git 插件);18 个分类筛选 |
| **切换与保存** | `空格` 暂存启停变更,`s` 预览 diff,`Enter` 写入 —— 原子写入,必定先备份 |
| **中文词典** | 359 条精编词典编译进二进制:简介 + 实战用法 + 常用别名中文注解 |
| **别名索引** | 别名直接从插件源码提取 —— 自定义插件同样适用 |
| **幽灵插件清理** | zshrc 里启用但磁盘上已不存在的残留(`x` 一键检测、标记移除,仍走 diff 确认;`omz-pm list` 也会给出警告) |
| **README 阅读器** | 在 TUI 里以渲染后的 markdown 读任意插件 README(表格对齐、标题/链接/代码块着色)——全部 359 个内置插件都带有精心翻译的整篇中文译文;自定义插件回退为在线轻翻译 |
...
TUI 里按 `B` 不用退出就能跑同样的分析(带实时进度与耗时分布图)。
...
```bash
cargo test      # 80 个单元测试(zshrc 往返 / 别名解析 / diff / 词典 / README 译文包 / 排版)
cargo clippy    # 零警告
```
